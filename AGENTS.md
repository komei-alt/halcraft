...
**ハルクラ**は、7歳のハルが描いた素材を原案とし、ハル由来の3Dモデルと高品質に再制作したテクスチャ・イラストを組み合わせる、オリジナルのマインクラフト風 3D ボクセルサンドボックスゲーム。
ブラウザ上で動作し、**デスクトップ（キーボード＋マウス）とモバイル（タッチ操作）の両方**に対応する。

### コンセプト

- **モデルはハルの素材を維持する**: キャラクター、モブ、乗り物、アイテムなどの3Dモデルは、ハルが描いた形・モチーフ・個性を正本として維持する
- **テクスチャとイラストは高品質化する**: ハルの素材をコンセプト原案として、世界観を保ちながら高解像度化・描き直し・生成・質感追加を行う
...
1. **モデルはハル素材を正本とする**: 3Dモデルの形状・シルエット・モチーフはハルの素材を維持し、無関係な既製・生成モデルへ置き換えない
2. **テクスチャは高品質再制作を許可する**: ハルの色・モチーフ・世界観をコンセプトとして、高解像度化、シームレス化、描き直し、生成、マテリアル表現の追加を行える
3. **イラストは高品質再制作を許可する**: UI、背景、演出用イラストは、ハルの原案が判別できる個性を保ちながら構図・線・彩色・解像度を刷新できる
4. **品質基準は「忠実な複製」ではなく「世界観の継承」**: 原画像の画素や手描き線をそのまま残すことより、ハルらしい発想・色・物語を高品質なゲーム表現へ昇華することを優先する
5. **コンセプトアート**: 世界観の参考資料として、スタイル設計や生成・再制作の指針に使用する（ゲーム内へそのまま直接使用しない）
...
- コンセプトアート → 世界観の参考。高品質なテクスチャ・イラストを再制作する際のスタイルや色味の指針
...
- 3Dモデルの形状・シルエット・モチーフは、ハル由来の素材を維持する
- テクスチャはハルの素材をコンセプト原案として、高解像度化、再描画、生成、法線・粗さ・発光等の質感追加を行える
- UI・背景・演出イラストも、ハルらしいモチーフと世界観を維持した上で高品質に再制作できる
- 汎用素材へ寄せてハルクラ固有の個性が失われる変更は禁止する
