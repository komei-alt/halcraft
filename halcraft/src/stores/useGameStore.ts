...
import { useModeFlowStore } from './useModeFlowStore';
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  startGame: () => {
    ...
    useModeFlowStore.getState().startRun(currentStage?.id ?? null);
    useFunctionalBlockStore.getState().clearFunctionalBlocks();
    ...
    const startingCameraY = getTerrainHeight(PLAYER_SPAWN.x, PLAYER_SPAWN.z) + 2.7;
    ...
    usePlayerStore.setState({
      hp: player.maxHp,
      worldPosition: { x: PLAYER_SPAWN.x, y: startingCameraY, z: PLAYER_SPAWN.z },
      isDead: false,
      isDamageFlash: false,
